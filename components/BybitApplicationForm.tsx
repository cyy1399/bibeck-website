...
export function BybitApplicationForm() {
  ...
  if (state.status === "success") return <section className="border border-gold/40 bg-gold/[0.04] p-6 sm:p-10" role="status"><p className="eyebrow">申請已收到</p><h2 className="mt-4 text-3xl font-semibold text-white">03｜等待審核通知</h2><p className="mt-5 text-sm leading-7 text-secondary">BiBeck 將核對你的 Bybit UID 與推薦關係，審核結果會寄送至你填寫的 Email。案件編號：<strong className="text-white">{state.caseNumber}</strong>。</p><p className="mt-4 text-sm text-secondary">有其他問題？</p><div className="mt-5 flex flex-col gap-3 sm:flex-row"><a href={LINE_OFFICIAL_URL} target="_blank" rel="noopener noreferrer" className="cta-button button-primary">LINE 聯絡 BiBeck</a><Link href="/calculator" className="cta-button button-secondary">回到交易成本計算器</Link></div></section>;
