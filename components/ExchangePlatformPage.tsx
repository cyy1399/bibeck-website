...
import { HIGH_VOLUME_MAILTO, REBATE_APPLICATION_URL, REBATE_BACKOFFICE_URL } from "@/config/links";
      ...
      <section className="px-5 py-14 sm:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className={"mt-10 grid gap-7 border-l-2 bg-[#111] p-7 sm:p-9 lg:grid-cols-[1fr_auto] lg:items-center " + (isBybit ? "border-gold" : "border-white/18")}>
            ...
            {isBybit ? (
              <Link href={REBATE_APPLICATION_URL} className="cta-button button-primary" aria-label="前往 BiBeck Bybit 返傭申請頁">{actionLabels.rebateSignup}</Link>
            ) : (
        ...
        <section className="section-muted border-y border-white/10 px-5 py-20 sm:px-8">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-2">
              <article className="border border-white/12 bg-[#101010] p-6 sm:p-8"><p className="eyebrow">一般交易者</p><h3 className="mt-4 text-3xl font-semibold text-white">35% 標準返傭</h3><p className="mt-5 text-sm leading-7 text-secondary">符合資格的交易手續費 × 35%，就是 BiBeck 標準返傭的計算基礎。</p><div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3 border-y border-white/10 py-5 text-center"><div><strong className="font-mono text-xl">1,000 USDT</strong><span className="mt-1 block text-xs text-white/44">符合資格手續費</span></div><span className="text-gold">→</span><div><strong className="font-mono text-2xl text-gold">350 USDT</strong><span className="mt-1 block text-xs text-white/44">標準返傭</span></div></div><Link href={REBATE_APPLICATION_URL} className="cta-button button-primary mt-6">{actionLabels.rebateSignup}</Link></article>
              <article className="border border-gold/40 bg-gold/[0.04] p-6 sm:p-8"><p className="eyebrow">高交易量 / 專業交易</p><h3 className="mt-4 text-3xl font-semibold text-white">交易量越大，越值得重新檢視交易成本。</h3><p className="mt-5 text-sm leading-7 text-secondary">如果你有較高交易量、Quant、Bot、做市或其他專業交易需求，BiBeck 可依實際交易結構進一步評估。</p><ul className="mt-5 grid gap-2 text-sm text-white/68">{["VIP 與費率結構","Maker / Taker 成本","實際有效費率","高交易量合作需求","專屬條件評估"].map((item)=><li key={item}>— {item}</li>)}</ul><a href={HIGH_VOLUME_MAILTO} target="_blank" rel="noopener noreferrer" className="cta-button button-secondary mt-6">LINE 洽談高交易量方案</a></article>
            </div>
      ...
      {isBybit ? (
        <section className="px-5 py-20 sm:px-8">
          <div className="mx-auto max-w-5xl">
            ...
            <div className="mt-8"><FAQList items={bybitFaqs} grouped /></div>
          </div>
        </section>
      ) : null}
