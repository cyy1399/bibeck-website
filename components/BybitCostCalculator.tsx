...
function SavingsSummary({result,displayMoney}:{result:ReturnType<typeof calculateTradingCostComparison>;displayMoney:(value:number)=>string}) { return <div className="savings-summary"><p className="text-sm text-secondary">按照目前輸入條件</p><dl className="mt-5 grid gap-4 sm:grid-cols-2"><div><dt className="text-xs text-white/46">30 日交易手續費</dt><dd className="mt-1 font-mono text-lg">{displayMoney(result.vipFee)}</dd></div><div><dt className="text-xs text-white/46">BiBeck {formatBibeckRebateRate()} 返傭</dt><dd className="mt-1 break-words font-mono text-3xl font-semibold text-gold">{displayMoney(result.rebateAmount)}</dd></div><div><dt className="text-xs text-white/46">返傭後實際交易成本</dt><dd className="mt-1 font-mono text-lg">{displayMoney(result.netTradingCost)}</dd></div><div><dt className="text-xs text-white/46">30 日總節省</dt><dd className="mt-1 font-mono text-lg">{displayMoney(result.totalSavings)}</dd></div></dl><p className="mt-5 text-sm text-white/68">年度預估可降低成本 {displayMoney(result.annualTotalSavings)}</p></div>; }
