...
import { BIBECK_REBATE_ELIGIBILITY_NOTICE, getStandardBibeckRebateRate } from "@/lib/bibeck-rebate";
import { getEstimatedTraderStatus, getNextTraderStatus, getTraderStatusProgress } from "@/lib/bibeck-trader-status";
...
export function BybitCostCalculator({ compact = false }: { compact?: boolean }) {
  ...
  const [customRebatePercent, setCustomRebatePercent] = useState(40);
  ...
  const estimatedStatus = getEstimatedTraderStatus(volume);
  const nextStatus = getNextTraderStatus(volume);
  const statusProgress = getTraderStatusProgress(volume);
  ...
  return <div className="bybit-calculator">
    ...
    <div className="grid gap-8 xl:grid-cols-[0.88fr_1.12fr]">
      <fieldset className="grid content-start gap-5"><legend className="sr-only">Bybit 交易成本計算器輸入欄位</legend>
        ...
        <div className="mode-field"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-semibold">BiBeck 標準返傭</p><button type="button" className="text-xs text-gold underline" onClick={()=>setRebateMode((mode)=>mode==="standard"?"custom":"standard")} aria-expanded={rebateMode==="custom"}>進階情境試算</button></div>{rebateMode === "custom" ? <div className="mt-4"><NumberInput label="模擬返傭比例（%）" value={customRebatePercent} setValue={setCustomRebatePercent}/><p className="mt-3 text-xs leading-6 text-white/44">此功能僅用於模擬不同假設情境，不代表 BiBeck 實際核准比例。</p></div> : <div className="mt-4"><p className="text-3xl font-semibold text-gold">40%</p><p className="mt-3 text-xs leading-6 text-white/48">{BIBECK_REBATE_ELIGIBILITY_NOTICE}</p></div>}</div>
        <div className="mode-field"><p className="text-sm font-semibold">推估 Trader Status</p><p className="mt-4 text-xl font-semibold text-gold">{estimatedStatus.name}</p><p className="mt-2 text-xs leading-6 text-white/48">以下身分依目前輸入的最近 30 日交易量推估。正式 Trader Status 需以 BiBeck 最終確認結果為準。</p><TierProgress label="Trader Status 進度" current={`${estimatedStatus.name}｜${formatVolume(volume)} USDT`} next={nextStatus ? `下一級 ${nextStatus.name}｜尚差 ${formatVolume(statusProgress.remaining)} USDT` : "已達 Black Trader Status 對應交易量里程碑；更大型合作需求可洽談 Partner。"} percentage={statusProgress.percentage}/></div>
        <ModeField title="手續費率" mode={customFeeEnabled?"manual":"auto"} setMode={(mode)=>setCustomFeeEnabled(mode==="manual")}>{customFeeEnabled ? <NumberInput label="自訂 VIP 費率（%）" value={customFeePercent} setValue={setCustomFeePercent}/> : <p className="text-sm">目前套用：<strong className="text-gold">{percent(vipRate)}</strong></p>}</ModeField></> : <div className="border-l-2 border-gold bg-black/20 p-4 text-sm leading-7 text-white/72"><p>推估 VIP：<strong className="text-gold">{estimatedVip.label}</strong></p><p>BiBeck 標準返傭：<strong className="text-gold">40%</strong></p></div>}
      </fieldset>
      <section className="comparison-results min-w-0" aria-live="polite"><p className="eyebrow">交易成本比較</p>{compact ? <><div className="mt-5 grid gap-px bg-white/10 sm:grid-cols-3">{planOrder.map((plan)=><article key={plan} className={`bg-[#141414] p-5 ${plan==="bibeck"?"border border-gold/50":""}`}><p className="text-xs text-gold">{planMeta[plan].code}</p><h3 className="mt-2 font-semibold">{planMeta[plan].title}</h3><p className="mt-4 text-xs text-white/44">30 日預估成本</p><p className="mt-2 break-words font-mono text-lg">{plans[plan].actualCost}</p></article>)}</div><CostBars bars={bars} displayMoney={displayMoney}/><SavingsSummary result={result} displayMoney={displayMoney}/></> : <><p className="comparison-swipe-hint mt-3 text-xs text-white/48">左右滑動比較三種成本</p><ComparisonMatrix plans={plans}/><MobileComparisonCards plans={plans}/><CostBars bars={bars} displayMoney={displayMoney}/><SavingsSummary result={result} displayMoney={displayMoney}/><ExternalLink href={REBATE_APPLICATION_URL} sponsored className="mt-6 w-full" aria-label="前往 BiBeck Bybit 返傭申請頁">取得 Bybit 返傭帳號</ExternalLink><p className="mt-3 text-xs leading-6 text-white/44">{BIBECK_REBATE_ELIGIBILITY_NOTICE}</p></>}</section>
    </div>
    {compact ? <div className="mt-7 text-center"><Link href="/calculator" className="button-secondary">查看完整成本分析</Link></div> : <div className="mt-8 border-t border-white/10 pt-7 text-xs leading-6 text-white/44"><details className="mb-6 border border-white/10 bg-black/20 p-4"><summary className="cursor-pointer font-semibold text-white">計算公式與年度推估方式</summary><div className="mt-3 grid gap-1"><p>原始手續費＝交易量 × VIP 0 基準費率</p><p>VIP 後手續費＝交易量 × 實際 VIP 費率</p><p>BiBeck 返傭＝VIP 後實際手續費 × 40%</p><p>返傭後成本＝VIP 後手續費 × 60%</p><p>合計節省＝原始手續費 − 返傭後成本</p><p>年度推估＝30 日結果 × 12</p></div></details><p>本試算依輸入資料與公開費率估算；實際結果可能受商品、地區、帳戶資格、活動、有效手續費及 Bybit 最終紀錄影響。</p><p className="mt-2">Trader Status 僅為依輸入交易量的推估，不代表網站已同步帳戶資料或自動完成身分確認。</p><p className="mt-2">BiBeck 為獨立第三方平台，並非 Bybit 官方網站或代表。返傭不是投資收益，也不保證所有帳戶或交易都適用。</p><p className="mt-2">費率來源：<ExternalLink href={BYBIT_FEE_STRUCTURE} variant="ghost" className="!min-h-0 !px-0 !py-0 !tracking-normal">Bybit Trading Fee Structure</ExternalLink></p></div>}
  </div>;
}
...
function SavingsSummary({result,displayMoney}:{result:ReturnType<typeof calculateTradingCostComparison>;displayMoney:(value:number)=>string}) { return <div className="savings-summary"><p className="text-sm text-secondary">30 日預估少付</p><p className="mt-2 break-words text-4xl font-semibold text-gold sm:text-5xl">{displayMoney(result.totalSavings)}</p><p className="mt-4 text-sm text-white/68">年度預估少付 {displayMoney(result.annualTotalSavings)}</p></div>; }
