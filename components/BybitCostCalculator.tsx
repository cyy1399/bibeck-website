...
import { BYBIT_FEE_STRUCTURE, HIGH_VOLUME_MAILTO, REBATE_APPLICATION_URL } from "@/config/links";
...
function HighVolumeCallout({volume}:{volume:number}) { return <aside className="mt-7 border border-gold/40 bg-gold/[0.05] p-6"><p className="eyebrow">高交易量交易者</p><h3 className="mt-3 text-xl font-semibold">你的交易量，值得進一步評估交易成本。</h3><p className="mt-4 text-sm leading-7 text-secondary">你輸入的最近 30 日交易量為 <strong className="text-white">{formatVolume(volume)} USDT</strong>。在這個交易量級，Maker / Taker、VIP 與實際有效費率的微小差異，都可能產生明顯的成本影響。</p><p className="mt-3 text-sm leading-7 text-secondary">除了 BiBeck 標準 35% 返傭，你也可以申請高交易量合作條件評估；實際條件由 BiBeck 個別確認。</p><a href={HIGH_VOLUME_MAILTO} className="cta-button button-secondary mt-5">洽談高交易量方案</a></aside>; }
