...
export function BybitCostCalculator() {
  ...
  const common = { volume: formatConvertedCurrency(volume, currency), baselineFee: displayMoney(result.baselineFee) };
  const rowsA: [string, string][] = [["方案", "一般狀況"], ["VIP 等級", "無 VIP"], ["BiBeck 級距", na], ["手續費率", percent(baselineRate)], ["30 日交易量", common.volume], ["30 日原始手續費", common.baselineFee], ["VIP 節省", na], ["BiBeck 返傭比例", na], ["30 日返傭金額", na], ["30 日實際交易成本", common.baselineFee], ["30 日合計節省", na], ["年度預估成本", displayMoney(result.annualBaselineCost)], ["年度預估節省", na], ["實際有效費率", percent(baselineRate)], ["總成本降低比例", na]];
  const rowsB: [string, string][] = [["方案", "VIP"], ["VIP 等級", selectedVip.label], ["BiBeck 級距", na], ["手續費率", percent(vipRate)], ["30 日交易量", common.volume], ["30 日原始手續費", common.baselineFee], ["VIP 節省", displayMoney(result.vipSavings)], ["BiBeck 返傭比例", na], ["30 日返傭金額", na], ["30 日實際交易成本", displayMoney(result.vipFee)], ["30 日合計節省", displayMoney(result.vipSavings)], ["年度預估成本", displayMoney(result.annualVipCost)], ["年度預估節省", displayMoney(result.annualVipSavings)], ["實際有效費率", percent(vipRate)], ["總成本降低比例", result.baselineFee ? percent(result.vipSavings / result.baselineFee) : "0%"]];
  const rowsC: [string, string][] = [["方案", "VIP＋BiBeck 返傭"], ["VIP 等級", selectedVip.label], ["BiBeck 級距", rebateMode === "auto" ? estimatedTier.name : "自訂情境"], ["手續費率", percent(vipRate)], ["30 日交易量", common.volume], ["30 日原始手續費", common.baselineFee], ["VIP 節省", displayMoney(result.vipSavings)], ["BiBeck 返傭比例", percent(rebateRate)], ["30 日返傭金額", displayMoney(result.rebateAmount)], ["30 日實際交易成本", displayMoney(result.netTradingCost)], ["30 日合計節省", displayMoney(result.totalSavings)], ["年度預估成本", displayMoney(result.annualNetCost)], ["年度預估節省", displayMoney(result.annualTotalSavings)], ["實際有效費率", percent(result.effectiveFeeRate)], ["總成本降低比例", percent(result.totalSavingsPercent)]];
  ...
  return <div className="bybit-calculator">
    ...
    <div className="grid gap-8 xl:grid-cols-[0.88fr_1.12fr]">
      <fieldset className="grid content-start gap-5"><legend className="sr-only">Bybit 交易成本計算器輸入欄位</legend>
        ...
        <label><span className="flex justify-between text-sm font-medium text-white">最近 30 日交易量 <span className="text-xs text-white/42">{currency}</span></span><FormattedNumberInput key={currency} ariaLabel="最近 30 日交易量" placeholder="例如 10,000,000 或 10M" value={displayVolume} onChange={(v) => setVolume(convertCurrency(v, currency, "USDT") ?? 0)} /><span className="mt-2 block text-xs leading-6 text-white/44">可輸入數字、千分位或 M；M 代表百萬，例如 10M USDT＝10,000,000 USDT。</span></label>
        ...
        <div className="mode-field"><p className="text-sm font-semibold">BiBeck 返傭</p><div className="mode-toggle mt-3" role="group" aria-label="BiBeck 返傭試算模式"><button type="button" aria-pressed={rebateMode === "auto"} onClick={() => setRebateMode("auto")}>依交易量自動推估</button><button type="button" aria-pressed={rebateMode === "custom"} onClick={() => setRebateMode("custom")}>自訂情境試算</button></div>{rebateMode === "custom" ? <div className="mt-4"><Select label="模擬返傭比例" value={String(customRebatePercent)} onChange={(v) => setCustomRebatePercent(Number(v))}>{[0,20,25,30,35].map((rate) => <option key={rate} value={rate}>{rate}%</option>)}<option value="40">自訂 40% 情境</option></Select><NumberInput label="自訂比例" value={customRebatePercent} setValue={setCustomRebatePercent} /><p className="mt-3 text-xs leading-6 text-gold">特殊合作情境：自訂比例僅供情境試算，不代表 BiBeck 實際可提供或核准的返傭比例。</p></div> : <div className="mt-4 text-sm leading-7 text-white/72"><p>目前推估級距：<strong className="text-gold">{estimatedTier.name}</strong></p><p>推估返傭比例：<strong className="text-gold">{percent(estimatedTier.rebateRate ?? 0)}</strong></p><p>適用交易量：{estimatedTier.volumeLabel}</p><p>{nextTier ? <>下一級：{nextTier.name}｜{percent(nextTier.rebateRate ?? 0)}；距離下一級：{formatVolume(nextProgress.remaining)} USDT</> : "已達最高公開交易量級距"}</p><p className="mt-2 text-xs text-white/42">此為級距推估，實際比例須完成返傭申請及資料核對後，以通知結果為準。</p></div>}</div>
...
function ScenarioCard({ code, title, rows, featured = false }: { code: string; title: string; rows: [string,string][]; featured?: boolean }) { return <article className={`scenario-card ${featured ? "scenario-card-featured" : ""}`} tabIndex={0}><div className="flex items-center justify-between gap-3"><span className="font-mono text-xs text-gold">{code}</span>{featured ? <span className="border border-gold/50 px-2 py-1 text-[0.68rem] text-gold">推薦</span> : null}</div><h3 className="mt-4 text-xl font-semibold text-white">{title}</h3><dl className="mt-5 divide-y divide-white/8">{rows.map(([label,value]) => <div key={label} className="grid min-h-14 grid-cols-[1fr_auto] items-center gap-3 py-3"><dt className="text-xs leading-5 text-white/44">{label}</dt><dd className="break-words text-right font-mono text-xs leading-5 text-white">{value}</dd></div>)}</dl></article>; }
