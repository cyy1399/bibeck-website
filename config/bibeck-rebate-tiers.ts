export type BiBeckRebateTierId = "standard" | "active" | "elite" | "core" | "strategic" | "special";
...
export const BIBECK_REBATE_TIERS = [
  ...
  { id: "core", name: "核心交易者", minVolume: 200_000_000, maxVolume: 500_000_000, rebateRate: 0.35, shortLabel: "35%", volumeLabel: "200M～499.99M USDT", description: "最近 30 日有效交易量 200M～499.99M USDT", requiresReview: true, isSpecial: false, order: 4 },
  { id: "strategic", name: "戰略交易者", minVolume: 500_000_000, maxVolume: null, rebateRate: 0.4, shortLabel: "40%", volumeLabel: "500M USDT 以上", description: "最近 30 日有效交易量達 500M USDT", requiresReview: true, isSpecial: false, order: 5 },
  { id: "special", name: "特殊合作方案", minVolume: null, maxVolume: null, rebateRate: null, shortLabel: "40% 以上／個別協商", volumeLabel: "不依交易量自動取得", description: "適用代理、社群、團隊或其他合作需求，須個別評估與協商", requiresReview: true, isSpecial: true, order: 6 },
...
export function formatVolume(value: number): string {
  ...
  if (safe >= 1_000_000_000) {
    const billions = safe / 1_000_000_000;
    return `${billions.toFixed(2).replace(/\.00$/, "").replace(/(\.\d)0$/, "$1")}B`;
  }
  if (safe >= 1_000_000) {
