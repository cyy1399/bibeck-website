...
export default async function AdminRequestsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <main className="min-h-screen bg-[#0a0a0a] px-5 py-12 text-white"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-6 overflow-x-auto border border-white/10"><table className="w-full min-w-[1180px] text-left text-sm"><thead className="bg-white/5 text-secondary"><tr>{["名稱或稱呼", "Bybit UID", "Email", "交易量", "申請時間", "返傭比例", "狀態", "操作"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/10">{cases.map((item) => <tr key={item.id} className={item.notificationStatus === "FAILED" ? "bg-red-950/20" : ""}>
      ...
      <td className="px-4">{item.contactEmail}<span className="ml-2"><CopyButton value={item.contactEmail} label=" Email" /></span></td><td className="px-4">{item.volumeRange}</td>
