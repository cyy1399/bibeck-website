...
export default function RebatePage() {
  return <SiteShell>
    ...
    <section className="section-muted border-y border-white/10 px-5 py-20 sm:px-8"><div className="mx-auto max-w-7xl"><SectionTitle label="Standard & High Volume" title="標準 35%，高交易量可進一步評估。" copy="35% 是 BiBeck 一般符合資格帳戶的正式標準產品，不會因輸入交易量自動改變。"/><div className="mt-10 grid gap-6 lg:grid-cols-2"><article className="border border-white/12 bg-[#121212] p-6 sm:p-8"><p className="eyebrow">BiBeck Standard</p><h2 className="mt-4 text-2xl font-semibold">一般符合資格帳戶：35% 標準返傭</h2><p className="mt-4 text-sm leading-7 text-secondary">返傭依成功開通並符合返傭條件的 Bybit 帳戶實際產生之符合資格交易手續費計算。</p><ExternalLink href={REBATE_APPLICATION_URL} sponsored className="mt-6">取得 35% 返傭帳戶</ExternalLink></article><article className="border border-gold/40 bg-gold/[0.04] p-6 sm:p-8"><p className="eyebrow">High Volume / Professional</p><h2 className="mt-4 text-2xl font-semibold">高交易量與專業交易需求</h2><p className="mt-4 text-sm leading-7 text-secondary">如果你的交易量較高，或使用 Quant、Bot、做市等專業交易方式，可以進一步洽詢交易成本與專屬合作條件評估；實際條件由 BiBeck 個別確認。</p><Link href="/partners#high-volume" className="button-secondary mt-6">洽談高交易量方案</Link></article></div></div></section>
